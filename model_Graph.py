...
def initialize_parameters():
    ...
    x_w1 = tf.get_variable("x_w1", [162,256], initializer = tf.contrib.layers.xavier_initializer(seed = 1))#创建新的tensorflow变量

    x_b1 = tf.get_variable("x_b1", [256], initializer = tf.zeros_initializer())

    x_w2 = tf.get_variable("x_w2", [256,2], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
...
def mynetwork(x, parameters, Lap, isTraining, momentums = 0.9):
    ...
    with tf.name_scope("x_layer_1"):
         ...
         x_z1_bn_out = tf.layers.dropout(x_z1_bn, rate=0.5, training=True)
         x_a1 = tf.nn.relu(x_z1_bn_out)
...
def train_mynetwork(x_train, x_test, y_train, y_test, L_train, L_test, 
                    learning_rate_base = 0.0001, beta_reg = 0.0001, num_epochs = 100, minibatch_size = 32, print_cost = True):
...
Train_X = scio.loadmat('.\Input\HYDICE\Train_X.mat')
TrLabel = scio.loadmat('.\Input\HYDICE\TrLabel.mat')
Test_X = scio.loadmat('.\Input\HYDICE\Test_X.mat')
TeLabel = scio.loadmat('.\Input\HYDICE\TeLabel.mat')
Train_L = scio.loadmat('.\Input\HYDICE\Train_L.mat')
Test_L = scio.loadmat('.\Input\HYDICE\Test_L.mat')
...
time_end=time.time()
print('time cost', time_end-time_start,'s')
sio.savemat('.\Features\gcnfeature.mat', {'features': features})