...
def generate_image_grid(sess, op):
    ...
    for i, g in enumerate(gs):
        ...
        img = np.array(x.tolist()).reshape(data.shape[0], data.shape[1])
...
def encoder(x, reuse=False):
    ...
    with tf.name_scope('Encoder'):
        ...
        return latent_variable_1,e_weights
...
def discriminator(x, reuse=False):
    ...
    with tf.name_scope('Discriminator'):
        ...
        cam_variance = global_variance_pooling(dc_den1)
        dc_den2 = tf.multiply(cam_variance,dc_den1)
        output, _ , _ = dense(dc_den2, n_l1, 1, name='dc_output')
...
def discriminator_1(x, reuse=False):
    ...
    with tf.name_scope('Discriminator_1'):
        ...
        cam_variance = global_variance_pooling(dc_den1) 
        dc_den2 = tf.multiply(cam_variance,dc_den1)
        output, _ , _ = dense(dc_den2, n_l1, 1, name='da_output')
        return output
    ...
    with tf.Session() as sess:
        if train_model:
            ...
            for i in range(1,len(meta_l)):
                ...
                scio.savemat(test_path+'HYloss_%d.mat'%(i), {'test_%d'%(i):output_y})
            print('finished')
